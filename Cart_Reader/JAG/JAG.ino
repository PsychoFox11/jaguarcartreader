...
byte romSize = 0; // 0 = 1MB, 1 = 2MB, 2 = 4MB, 3 = 6MB
...
static const char* const romMenu[] PROGMEM = {romItem1, romItem2, romItem3, romItem4};
...
void setup() {
  ...
  // Print all the info
  if (romSize > 3) { // ROM size out of range, set to default
    romSize = 1; // default 2MB
    EEPROM_writeAnything(8, romSize);
  }  
  if (romSize < 3 ) { // ROM is less than 6MB, romSize used to calculate cartSize
    cartSize = long(int_pow(2, romSize)) * 0x100000;
  } else { // romSize is 3, set to 6MB
    cartSize = 0x600000;
  }
